 ...
 public class TaskProperty {
	 ...
	 public TaskProperty(int _mobileDeviceId, int _taskType, double _startTime, ExponentialDistribution[][] expRngList) {
		 ...
		 if (taskLookupTable instanceof double[][]) {
			 ...
			 if (taskType < lookupTable.length && lookupTable[taskType].length > 8) {
				 pesNumber = (int)lookupTable[taskType][8];
			 } else {
				 pesNumber = 1; // 默认值
				 System.err.println("警告: 无法从任务查询表获取PES数量，使用默认值1");
			 }
		 } else {
			 pesNumber = 1; // 默认值
			 System.err.println("警告: 任务查询表类型不正确，使用默认PES数量1");
		 }
	 }
	 ...
	 public TaskProperty(int mobileDeviceId, double startTime, ExponentialDistribution[] expRngList) {
		 ...
		 if (taskLookupTable instanceof double[][]) {
			 double[][] lookupTable = (double[][])taskLookupTable;
			 if (lookupTable.length > 0 && lookupTable[0].length > 8) {
				 pesNumber = (int)lookupTable[0][8];
			 } else {
				 pesNumber = 1; // 默认值
				 System.err.println("警告: 无法从任务查询表获取PES数量，使用默认值1");
			 }
